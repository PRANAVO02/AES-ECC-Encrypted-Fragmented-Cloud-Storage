...
def encrypt_and_upload(file_path):
    ...
    # Encrypt fragments with AES and upload to Dropbox
    ...
    # Create manifest entry for this file
    manifest_entry = {
    ...
    # Load existing registry and update
    registry = load_registry()  # loads existing manifest.json
    registry[filename] = manifest_entry
    save_registry(registry)     # update local manifest.json

    # Encrypt the entire manifest.json
    ...
    print(f"\n✅ File '{filename}' uploaded successfully with encrypted manifest.")
